...
def generer_graphique_projection(commentaire_plongement: pd.DataFrame,
                                 perplexite: float,
                                 distance: str,
                                 methode_projection: str) -> Figure:
    if commentaire_plongement.shape[0] < 10:
        return {}
    print("methode projection : ", methode_projection)
    if methode_projection == 't-SNE':
        ...
        coord_pca = pca.fit_transform(commentaire_plongement.loc[:, 'V_000':])
        principal_df = pd.DataFrame(data=coord_pca, columns=['PC1', 'PC2'])
        principal_df['titre'] = commentaire_plongement.titre.values
        figure = px.scatter(principal_df, x='PC1', y='PC2', color='titre', hover_data=principal_df.columns)
