...
application.layout = dbc.Container(
    [
        ...
        dcc.Store(id='Liste_indices', storage_type='session', data={})
    ],
    fluid=True,
)
...
def mettre_a_jour_figure_projection(nb_max_commentaire_par_film: int, min_max_mot: tuple[int, int],
    ...
    figure, indices = generer_graphique_projection(df_filtre, perplexite=perplexite, distance=distance,
                                                   selected_method=selected_method)

    return figure, {'indices': indices}
...
def mettre_a_jour_figure_prediction(
        donnees_partage: dict, liste_indices: tuple, selected_method: str) -> go.Figure:
...
def mettre_a_jour_figure_prediction(donnees_partage: dict, liste_indices: dict) -> go.Figure:
    if donnees_partage:
        plongement = pd.read_json(StringIO(donnees_partage['plongement']), orient='split').to_numpy()
